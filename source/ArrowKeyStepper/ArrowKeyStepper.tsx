...
class ArrowKeyStepper extends React.PureComponent<Props, State> {
	...
	_onKeyDown: React.KeyboardEventHandler<HTMLDivElement> = (event) => {
		...
		switch (event.key) {
			case 'ArrowDown':
				scrollToRow =
					mode === 'cells' ? Math.min(scrollToRow + 1, rowCount - 1) : Math.min(this._rowStopIndex + 1, rowCount - 1)
				break
	...
	_onSectionRendered = ({ columnStartIndex, columnStopIndex, rowStartIndex, rowStopIndex }: RenderedSection) => {
