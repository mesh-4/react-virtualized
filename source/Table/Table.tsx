...
export default class Table extends React.PureComponent {
		...
		return (
			<div
				...
				<Grid
					...
					style={{ ...gridStyle, overflowX: 'hidden' }}
				/>
			</div>
		)
	}
	...
	_createHeader({ column, index }) {
		...
		const style = this._getFlexStyleForColumn(column, { ...headerStyle, ...column.props.headerStyle })
	...
	_createRow({ rowIndex: index, isScrolling, key, parent, style }) {
		...
		const columns = React.Children.toArray(children).map((column, columnIndex) =>
			this._createColumn({
				column,
				columnIndex,
				isScrolling,
	...
	_getFlexStyleForColumn(column, customStyle = {}) {
		...
		const style = { ...customStyle, flex: flexValue, msFlex: flexValue, WebkitFlex: flexValue }
