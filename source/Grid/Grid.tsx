...
class Grid extends React.PureComponent<Props, State> {
	...
	handleScrollEvent({ scrollLeft: scrollLeftParam = 0, scrollTop: scrollTopParam = 0 }: ScrollPosition) {
		...
		if (this.state.scrollLeft !== scrollLeft || this.state.scrollTop !== scrollTop) {
			...
			this.setState({
				...
				scrollPositionChangeReason: SCROLL_POSITION_CHANGE_REASONS.OBSERVED as 'observed',
				needToResetStyleCache: false,
				...(!autoHeight && { scrollTop }),
				...(!autoWidth && { scrollLeft }),
			})
		}
	...
	scrollToCell({ columnIndex, rowIndex }: CellPosition) {
		...
		if (columnCount > 1 && columnIndex !== undefined) {
			this._updateScrollLeftForScrollToColumn({ ...props, scrollToColumn: columnIndex })
		}
	...
	componentDidMount() {
		...
		if ((typeof scrollLeft === 'number' && scrollLeft >= 0) || (typeof scrollTop === 'number' && scrollTop >= 0)) {
			...
			if (stateUpdate) {
				...
				this.setState(stateUpdate as unknown as State)
			}
		}
	...
	static _getScrollLeftForScrollToColumnStateUpdate(nextProps: Props, prevState: State): Partial<State> {
	...
	static _getScrollTopForScrollToRowStateUpdate(nextProps: Props, prevState: State): Partial<State> {
