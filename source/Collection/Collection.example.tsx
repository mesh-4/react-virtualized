...
export default class CollectionExample extends React.PureComponent {
	...
	render() {
		const { cellCount, height, horizontalOverscanSize, scrollToCell, showScrollingPlaceholder, verticalOverscanSize } =
			this.state
		return (
			<ContentBox>
				<ContentBoxHeader
				...
				<ContentBoxParagraph>
					Renders scattered or non-linear data. Unlike <code>Grid</code>, which renders checkerboard data,{' '}
					<code>Collection</code> can render arbitrarily positioned- even overlapping- data.
				</ContentBoxParagraph>
				...
				<InputRow>
					<LabeledInput label="Num cells" name="cellCount" onChange={this._onCellCountChange} value={cellCount} />
					<LabeledInput
						...
						value={scrollToCell || ''}
					/>
					<LabeledInput label="Height" name="height" onChange={this._onHeightChange} value={height} />
					<LabeledInput
