...
type RenderCallback = (index: number, left: number, top: number) => void // Position cache requirements:
//   O(log(n)) lookup of cells to render for a given viewport size
//   O(1) lookup of shortest measured column (so we know when to enter phase 1)

export default class PositionCache {
	// Tracks the height of each column
	_columnSizeMap: Record<number, number> = {}
	// Store tops and bottoms of each cell for fast intersection lookup.
	_intervalTree = createIntervalTree()
	// Maps cell index to x coordinates for quick lookup.
	_leftMap: Record<number, number> = {}
	...
	get shortestColumnSize(): number {
		...
		for (let i in columnSizeMap) {
			let height = columnSizeMap[i as any]
