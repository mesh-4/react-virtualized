...
describe('ColumnSizer', () => {
	function getMarkup({ columnMinWidth = undefined, columnMaxWidth = undefined, columnCount = 10, width = 200 } = {}) {
				...
				{({ adjustedWidth, columnWidth, registerChild }) => (
					<div>
						<Grid
							...
							width={adjustedWidth}
						/>
						<div className="debug">{`adjustedWidth:${adjustedWidth} columnWidth:${columnWidth}`}</div>
					</div>
				)}
			</ColumnSizer>
		)
	...
	it('should error if the registered child is not a Grid or a MultiGrid', () => {
		...
		expect(() => {
			render(
				<ColumnSizer columnMinWidth={100} columnMaxWidth={100} columnCount={100} width={100}>
