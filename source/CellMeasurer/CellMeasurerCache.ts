...
export default class CellMeasurerCache implements CellMeasureCache {
	...
	columnWidth = ({ index }: IndexParam) => {
		...
		return this._columnWidthCache[key] !== undefined ? this._columnWidthCache[key] : this._defaultWidth
	}
	...
	rowHeight = ({ index }: IndexParam) => {
		...
		return this._rowHeightCache[key] !== undefined ? this._rowHeightCache[key] : this._defaultHeight
	}
